...
resource "aws_vpc" "vpc_block" {
  ...
  tags = {
    Name = "mlops-zc-sd-vpc-block-${var.env}"
  }
}
...
resource "aws_internet_gateway" "igw_block" {
  ...
  tags = {
    Name = "mlops-zc-sd-igw-block-${var.env}"
  }
}
...
resource "aws_subnet" "public_subnet" {
  ...
  tags = {
    Name = "mlops-zc-sd-public-subnet-${count.index}-${var.env}"
  }
}
...
resource "aws_subnet" "private_subnet" {
  ...
  // suffixed with the count
  tags = {
    Name = "mlops-zc-sd-private-subnet-${count.index}-${var.env}"
  }
}
...
resource "aws_security_group" "ec2_sg" {
  ...
  name        = "mlops-zc-sd-ec2-sg-${var.env}"
...
resource "aws_security_group" "db_sg" {
  // Basic details like the name and description of the SG
  name        = "mlops-zc-sd-db-sg-${var.env}"
...
resource "aws_db_subnet_group" "db_subnet_group" {
  ...
  name        = "mlops-zc-sd-db-subnet-group-${var.env}"
...
resource "aws_db_instance" "database" {
  ...
  // Here we are tagging the database with the name "mlops-zc-sd-database"
  tags = {
    Name = "mlops-zc-sd-mlflow-database-${var.env}"
  }
}
...
resource "aws_instance" "ec2_instance" {
  ...
  // We are tagging the EC2 instance with the name "mlops-zc-sd-ec2-instance-" followed by
  // the count index
  tags = {
    Name = "mlops-zc-sd-ec2-instance-${count.index}-${var.env}"
  }
}
...
resource "aws_eip" "ec2_eip" {
  ...
  tags = {
    Name = "mlops-zc-sd-ec2-instance-eip-${count.index}-${var.env}"
  }
}